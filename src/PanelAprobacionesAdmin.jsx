...
import { Clock, History, CheckCircle2, XCircle, Package, ArrowRightLeft } from 'lucide-react';
...
export default function PanelAprobacionesAdmin({ movimientos, usuarioAdmin, onMovimientoProcesado }) {
  ...
  const historial = movimientos.filter(m => m.estado !== 'pendiente');
  ...
  const manejarAprobacion = async (movimientoId, nuevoEstado) => {
    ...
    try {
      ...
      // 2. Si se aprueba, transferir activos fijos a la bodega destino
      if (nuevoEstado === 'aprobado') {
        ...
        if (movActual) {
          ...
          if (activosATransferir && activosATransferir.length > 0) {
            await supabase
              .from('activos')
              .update({ bodega_id: movActual.destino_id || movActual.bodega_destino_id })
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                <div>
                  <span className="text-slate-400 text-[11px]">
                    {new Date(mov.fecha_solicitud).toLocaleString()} — Técnico: <strong className="text-slate-600">{mov.usuarios?.nombre || 'Desconocido'}</strong>
                  </span>
                  <p className="font-bold text-slate-800 text-sm mt-0.5">
                    De <span className="text-purple-700">{mov.origen?.nombre}</span> a <span className="text-purple-700">{mov.destino?.nombre}</span>
                  </p>
                </div>

                {/* Acciones para Admin / Estado de lectura para Técnico */}
                  ...
                  {mov.movimiento_detalles?.map((det) => (
                    <span key={det.id} className="bg-white text-slate-700 px-2.5 py-1 rounded-lg text-xs font-medium border border-slate-200/80 shadow-2xs flex items-center gap-1.5">
                      ...
                      {det.tipo_item === 'activo'
                        ? `${det.activos?.inventario || 'Activo'} (${det.activos?.categoria})`
